...
import { getActivity } from "@/data/dummy";
      ...
      <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
        <View style={{ paddingHorizontal: 20, paddingTop: 8 }}>
          <View style={styles.heroShadow}>
            <PlaceholderImage title={activity.name} aspectRatio={16 / 9} radius={18} fontSize={48} />
          </View>
        </View>
        <View style={{ paddingHorizontal: 20, marginTop: 20 }}>
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
            ...
            <View style={{ paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border }}>
              <Text style={{ fontSize: 11, color: colors.mutedForeground, fontFamily: "Inter_500Medium" }}>{activity.category}</Text>
            </View>
          </View>
          <Text style={{ fontSize: 28, fontFamily: "Inter_700Bold", color: colors.foreground, lineHeight: 34 }}>{activity.name}</Text>
          <Text style={{ fontSize: 15, color: colors.foreground, marginTop: 14, lineHeight: 23 }}>{activity.description}</Text>
          ...
          <View style={{ marginTop: 28 }}>
            <FullSafety scores={activity.safetyScores} />
          </View>
